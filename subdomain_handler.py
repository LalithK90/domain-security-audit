...
"""
subdomain_handler.py

Suggested new filename: `subdomain_enumerator.py` (or keep this name).

What this script does:
- Enumerates subdomains for a hard-coded `domain` in main() using crt.sh and light DNS probing.
- Classifies discovered hosts into all / resolved / active / created-but-not-active.
- Writes three files: <domain>_all.txt, <domain>_active.txt, <domain>_created_not_active.txt
...
How to run (from this folder):
    ...
    # run the enumerator (domain is set in the script's main function)
    python subdomain_handler.py

If you prefer a different name, rename the file to `subdomain_enumerator.py`.

Notes:
- The script performs network requests (crt.sh queries, DNS, HTTP), so run it where network access is permitted.
- The generated files will be created in the current working directory.
"""
