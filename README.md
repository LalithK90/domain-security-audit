...
├── generate_paper_tables.py      # Academic paper tables
...
```bash
# Just run again - it will skip completed targets
./run.sh
```
...
```bash
# Generate summary reports from latest run
python generate_reports.py --domain ac.lk

# Generate academic paper tables
python generate_paper_tables.py --domain ac.lk
```
...
**generate_paper_tables.py** produces LaTeX-ready tables:

- **Table 1**: Run and dataset overview
- **Table 2**: Top 10 most/least secure subdomains
- **Table 3**: Security control pass rates
- **Table 4**: Risk distribution
- **Table 5**: Enumeration method effectiveness
- **Table 6**: Error analysis

**Usage**:
```bash
python generate_paper_tables.py --domain ac.lk --output paper_tables.tex
```

### Report Generation

**generate_reports.py** produces comprehensive HTML/Excel reports:

- Executive summary
- Per-subdomain detailed findings
- Control-wise analysis
- Trend analysis (multi-run comparison)
- Recommendations

**Usage**:
```bash
python generate_reports.py --domain ac.lk --format html
python generate_reports.py --domain ac.lk --format excel
```
